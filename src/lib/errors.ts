...
export function formatErrorResponse(error: AppError, includeStack = false): Record<string, any> {
...
export function createExternalServiceError(service: string, originalError: any): ExternalServiceError {
...
export function createRateLimitError(limit: number, window: number): RateLimitError {
  return new RateLimitError(`Rate limit exceeded: ${limit} requests per ${window}ms`, {
    limit,
    window
  })
}
