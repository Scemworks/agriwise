...
export class CropRecommendationAI {
  ...
  async getRecommendations(
    ...
    try {
      ...
      for (const crop of this.cropDatabase) {
        const score = this.calculateCropScore(crop, soilData, weatherData, farmData)
        
        if (score > 0.3) { // Only recommend crops with >30% compatibility
          const recommendation = this.createRecommendation(crop, score, soilData, weatherData, farmData)
  ...
  private calculateCropScore(
    ...
    const tempScore = this.calculateTemperatureScore(crop.temperatureRange, weatherData.temperature)
    score += tempScore * 0.2
    factors += 0.2

    // Rainfall compatibility (15% weight)
    const rainfallScore = this.calculateRainfallScore(crop.rainfallRange, weatherData.rainfall)
    score += rainfallScore * 0.15
    factors += 0.15

    // Sunlight compatibility (10% weight)
    const sunlightScore = this.calculateSunlightScore(crop.sunlightHours, weatherData.sunlight)
    score += sunlightScore * 0.1
    factors += 0.1

    // Season compatibility (10% weight)
    const seasonScore = crop.plantingSeason.includes(weatherData.season) ? 1 : 0.3
    ...
    const irrigationScore = this.calculateIrrigationScore(crop.waterNeeds, farmData.irrigation)
  ...
  private calculateTemperatureScore(tempRange: [number, number], currentTemp: number): number {
  ...
  private calculateRainfallScore(rainfallRange: [number, number], currentRainfall: number): number {
  ...
  private calculateSunlightScore(sunlightRange: [number, number], currentSunlight: number): number {
  ...
  private calculateIrrigationScore(waterNeeds: string, hasIrrigation: boolean): number {
  ...
  private createRecommendation(
    ...
    const profitPotential = this.calculateProfitPotential(crop, farmData.landSize)
  ...
  private generateReasons(
    ...
    if (crop.phRange[0] <= soilData.ph && soilData.ph <= crop.phRange[1]) {
      reasons.push(`Soil pH (${soilData.ph}) is within optimal range (${crop.phRange[0]}-${crop.phRange[1]})`)
    }

    if (crop.temperatureRange[0] <= weatherData.temperature && weatherData.temperature <= crop.temperatureRange[1]) {
      reasons.push(`Current temperature (${weatherData.temperature}°C) is ideal for growth`)
    }
  ...
  private calculateProfitPotential(crop: CropData, landSize: number): 'low' | 'medium' | 'high' {
    const expectedRevenue = (crop.yieldPerAcre[0] + crop.yieldPerAcre[1]) / 2 * crop.marketPrice * landSize
  ...
  // Helper methods
  private getPlantingTime(crop: CropData, season: string): string {
  ...
  getCropDetails(cropName: string): CropData | null {
    return this.cropDatabase.find(crop => crop.name.toLowerCase() === cropName.toLowerCase()) || null
  }
}
