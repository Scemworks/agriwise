...
function getBaseUrl(): string {
  ...
  const useSandbox = process.env.NODE_ENV === 'development' && process.env.USE_WEATHER_SANDBOX === 'true'
...
async function callIndianApi(path: string, params?: Record<string, string>) {
  ...
  if (!res.ok) {
    ...
    const msg = detail ? `Indian API error ${res.status}: ${detail}` : `Indian API error ${res.status}: ${text}`
