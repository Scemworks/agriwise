...
async function handler(req: Request) {
  ...
  const { name, description, price, quantity, categoryId, images, auction, startingBid, buyNowPrice, endsAt, listing } = body
  ...
  const product = await prisma.product.create({
    data: {
      ...
      images: images && Array.isArray(images) && images.length > 0 ? { create: images.map((url: string) => ({ url })) } : undefined,
    },
  })
