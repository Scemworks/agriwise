...
async function handler(req: Request) {
  ...
  const id =
    pathParts[pathParts.length - 2] === "listings"
  ...
  const listing = await prisma.listing.findUnique({
    where: { id },
    include: { bids: true, product: { include: { seller: true } } },
  });
  if (!listing)
  ...
  const highest = listing.bids.reduce(
    (max: number, b: { amount: number }) => (b.amount > max ? b.amount : max),
    0,
  );
  if (listing.startingBid && amount < listing.startingBid)
    return NextResponse.json(
      { error: "Bid below starting bid" },
      { status: 400 },
    );
  if (amount <= highest)
    return NextResponse.json(
