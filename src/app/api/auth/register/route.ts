...
import { RegisterSchema, validateRequest } from "../../../../lib/security";
...
async function handler(req: Request) {
  try {
    ...
    const stateName: string | undefined = state;
    const cityName: string | undefined = city;

    logAuth("User registration attempt", undefined, { email, role });
    ...
    // Create farm only if user is a FARMER and farming details provided
    let farm = null;
    try {
      if (role === "FARMER" && (soilType || currentCrops || landSize)) {
      ...
      logAuth("Failed creating farm during registration", user.id, {
        error: msg,
      });
