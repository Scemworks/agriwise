...
const RESPONSES: Record<string, Record<string, string>> = {
  en: {
    ...
    fertilizer: 'Soil testing is recommended. Base fertilizer application on NPK needs — generally split nitrogen applications across growth stages.',
    default: 'Could you provide crop and location details so I can give tailored advice?'
  },
  hi: {
...
export async function handler(req: Request) {
  ...
  try {
    ...
    if (message.includes('pest') || message.includes('disease') || message.includes('कीट') || message.includes('रोग')) reply = dict.pest
    else if (message.includes('fertil') || message.includes('उर्वर')) reply = dict.fertilizer
    else if (message.includes('hi') || message.includes('hello') || message.includes('नमस्ते')) reply = dict.greeting
