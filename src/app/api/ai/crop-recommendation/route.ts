...
} from "../../../../lib/ai/crop-recommendation";
...
async function handler(req: Request) {
  ...
  try {
    ...
    const scoreCrop = (cropName: string) => {
      const crop = cropRecommendationAI.getCropDetails(
        cropName,
      ) as CropData | null;
      if (!crop) return 0;
      ...
      if (ph >= crop.phRange[0] && ph <= crop.phRange[1]) s += 2;
        ...
        s += Math.max(
          0,
          1 - Math.abs(ph - (crop.phRange[0] + crop.phRange[1]) / 2) / 2,
        );

      // temperature
      const temp = weatherData.temperature;
      if (temp >= crop.temperatureRange[0] && temp <= crop.temperatureRange[1])
        s += 2;
      else
        s += Math.max(
          0,
          1 -
            Math.abs(
              temp - (crop.temperatureRange[0] + crop.temperatureRange[1]) / 2,
            ) /
              10,
        );
    ...
    const scored = allCrops
      .map((c) => ({ name: c, score: scoreCrop(c) }))
    ...
    const recommendations = scored.slice(0, 5).map((s) => {
      ...
      const confidence = Math.max(
        0,
        Math.min(1, Math.round((s.score / 8) * 100) / 100),
      );
      const reasons: string[] = [];
      if (soilData.type && crop.soilTypes.includes(soilData.type))
        reasons.push(`Compatible with ${soilData.type} soil`);
      if (soilData.ph >= crop.phRange[0] && soilData.ph <= crop.phRange[1])
        reasons.push(`Soil pH ${soilData.ph} is optimal`);
      if (crop.plantingSeason.includes(weatherData.season))
        reasons.push(`Suitable planting season: ${weatherData.season}`);
