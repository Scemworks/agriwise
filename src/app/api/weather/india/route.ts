  ...
  try {
    ...
    if (!process.env.INDIANAPI_KEY) {
      return NextResponse.json(
        {
          error:
            "Server misconfiguration: INDIANAPI_KEY is not set. Add it to .env.local or your environment.",
        },
        { status: 500 },
      );
    }
    ...
    if (
      String(msg).includes("list index out of range") ||
      (remote && JSON.stringify(remote).includes("list index out of range"))
    ) {
      return NextResponse.json(
        {
          error:
            "Ambiguous city query. Try a more specific city name or call /api/weather/india/cities to find the correct IMD station.",
        },
        { status: 422 },
      );
    }
