...
export default function CarbonCreditsPage() {
  const { t } = useLanguage()
  const [activeTab, setActiveTab] = useState<'overview' | 'calculate' | 'sell' | 'history'>('overview')
  ...
  const totalCredits = mockCredits.reduce((sum, credit) => sum + credit.credits, 0)
  const totalValue = mockCredits.reduce((sum, credit) => sum + credit.value, 0)
  const activeCredits = mockCredits.filter(credit => credit.status === 'active').reduce((sum, credit) => sum + credit.credits, 0)
  ...
  const OverviewTab = () => (
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-3 gap-6">
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted">{t.totalCredits}</p>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted">{t.activeCredits}</p>
                <p className="text-2xl font-bold text-gray-900">{activeCredits}</p>
              </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className='text-gray-900'>{t.recentCarbonCredits}</CardTitle>
          <CardDescription className='text-muted'>{t.recentActivities}</CardDescription>
        </CardHeader>
        <CardContent>
        ...
        <CardContent>
          <div className="space-y-6">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <Card>
                <CardHeader>
                  <CardTitle className="text-lg text-gray-600">Available Credits</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="text-center">
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-lg text-gray-600">Market Price</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="text-center">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                {mockCredits.filter(credit => credit.status === 'sold').map((credit) => (
                  <div key={credit.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    <div>
                      <h4 className="font-medium text-gray-900">{credit.practice}</h4>
                      <p className="text-sm text-muted">{credit.date}</p>
                    </div>
                    <div className="text-right">
                      <div className="font-semibold text-green-600">₹{credit.value}</div>
                      <div className="text-sm text-muted">{credit.credits} credits</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="grid md:grid-cols-3 gap-6">
              <Card>
                <CardContent className="p-6 text-center">
                  ...
                  <h3 className="font-semibold text-gray-900 mb-2">Additional Income</h3>
                  <p className="text-sm text-muted">
                    Earn money from sustainable practices you're already implementing
                  </p>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardContent className="p-6 text-center">
                  ...
                  <p className="text-sm text-muted">
                    Contribute to climate change mitigation and environmental protection
                  </p>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardContent className="p-6 text-center">
                  ...
                  <h3 className="font-semibold text-gray-900 mb-2">Verified Practices</h3>
                  <p className="text-sm text-muted">
