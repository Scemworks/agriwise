...
export default function DashboardPage() {
  ...
  const _farms = data?.farms || [];
  const activities = data?.activities || [];
  const weatherData = data?.latestWeather
    ? {
        temperature: data.latestWeather.temperature
          ? `${data.latestWeather.temperature}°C`
          : "—",
        humidity: data.latestWeather.humidity
          ? `${data.latestWeather.humidity}%`
          : "—",
        rainfall: data.latestWeather.rainfall
          ? `${data.latestWeather.rainfall}mm`
          : "—",
        windSpeed: data.latestWeather.windSpeed
          ? `${data.latestWeather.windSpeed} km/h`
          : "—",
  ...
  const _carbonSummary = data?.carbonSummary || {
    _sum: { credits: null, value: null },
  };
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  {(activities.length ? activities : recentActivity).map(
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-center mb-4">
                  ...
                  <div className="text-sm text-muted">{t.creditsGenerated}</div>
                </div>
