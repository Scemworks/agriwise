...
async function testWeatherAPI() {
  ...
  try {
    ...
    if (citiesResponse.ok) {
      ...
      console.log(
        `✓ Indian Cities API working - Found ${Object.keys(cities).length} cities`,
      );
      console.log(
        `  Sample cities: ${Object.values(cities).slice(0, 3).join(", ")}`,
      );
    } else {
    ...
    const indiaWeatherResponse = await fetch(
      `${BASE_URL}/api/weather/india?city=Chennai`,
    );
    if (indiaWeatherResponse.ok) {
      const weather = await indiaWeatherResponse.json();
      console.log(
        `✓ Indian Weather API working - Got weather for ${weather.city}`,
      );
      console.log(
        `  Temperature: ${weather.weather?.current?.temperature?.max?.value || "N/A"}°C`,
      );
    } else {
      console.log(
        `✗ Indian Weather API failed: ${indiaWeatherResponse.status}`,
      );
    }

    // Test global weather endpoint
    console.log("\n3. Testing Global Weather API...");
    const globalWeatherResponse = await fetch(
      `${BASE_URL}/api/weather/global/weather?location=London&days=2`,
    );
    if (globalWeatherResponse.ok) {
      const weather = await globalWeatherResponse.json();
      console.log(
        `✓ Global Weather API working - Got weather for ${weather.location}`,
      );
      console.log(`  Temperature: ${weather.current?.temperature || "N/A"}°C`);
    } else {
      console.log(
        `✗ Global Weather API failed: ${globalWeatherResponse.status}`,
      );
    }
